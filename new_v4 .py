...
#         """Send a single text chunk to Gemini for PURE FORMATTING ONLY - NO CONTENT ALTERATION."""
#         if not text_chunk or not text_chunk.strip():
#             print(f"Skipping empty chunk {chunk_num}/{total_chunks}.")
#             return ""

#         # CRITICAL: Ultra-strict prompt to prevent any content alteration
#         content_preservation_prompt = """
#         YOU ARE A PURE TEXT FORMATTER. YOUR ONLY JOB IS TO ORGANIZE THE PROVIDED TEXT INTO A HIERARCHICAL OUTLINE FORMAT.

#         ABSOLUTE RULES - VIOLATION WILL RESULT IN REJECTION:
#         1. DO NOT change, alter, rephrase, summarize, or interpret ANY content
#         2. DO NOT add your own words, explanations, or interpretations
#         3. DO NOT remove or skip any sentences or paragraphs
#         4. DO NOT correct grammar, spelling, or language
#         5. USE EXACT ORIGINAL WORDING - copy sentences word-for-word
#         6. PRESERVE all citations, references, and technical terms exactly as written
#         7. MAINTAIN original sentence structure completely
#         8. INCLUDE all repetitive content exactly as it appears

#         YOUR ONLY TASK: Take the existing text and organize it using this hierarchy format:

#         FORMAT RULES:
#         1. Main Topic One
#         |-- [Exact sentence/paragraph from original text]
#         |-- [Another exact sentence/paragraph from original text]
#           1.b Subtopic (if natural division exists in original)
#           |-- [Exact sentence from original under this subtopic]
#           | |-- [Exact sentence if it's a sub-detail]
#         2. Main Topic Two
#         |-- [Exact sentence/paragraph from original text]
...
#         Original text: "Strategic planning involves multiple steps. First, assess current situation. Market analysis is crucial. External factors must be considered."
...
#         - Change "Strategic planning involves multiple steps" to "Strategic planning has several phases"
#         - Add explanations like "This section discusses strategic planning concepts"
#         - Skip any sentences
#         - Combine sentences into new phrases

#         Remember: You are ONLY a formatter, not a content creator or editor.
#         """

#         chunk_instruction = f"""
#         This is Chunk {chunk_num} of {total_chunks} from a larger document.
        
#         Format ONLY the content in THIS CHUNK. Do not add connecting text between chunks.
        
#         Here is the exact text content to format (DO NOT ALTER ANY WORDS):
        
#         ---
#         {text_chunk}
#         ---
        
#         Provide ONLY the formatted outline using exact original text. No additional commentary.
#         """

#         full_prompt = f"{content_preservation_prompt}\n\n{chunk_instruction}"
        
#         print(f"Sending Chunk {chunk_num}/{total_chunks} to Gemini for FORMATTING ONLY ({len(text_chunk)} chars)...")
#         try:
#             response = self.model.generate_content(
#                 full_prompt,
#                 generation_config=genai.types.GenerationConfig(
#                     temperature=0.1,  # Very low to minimize creativity/alteration
...
#                 print(f"Chunk {chunk_num} outline passed content preservation check.")
#                 return outline_output
#             else:
#                 print(f"WARNING: Chunk {chunk_num} outline FAILED content preservation check. Content may have been altered.")
#                 print("This chunk will be skipped to prevent data corruption.")
#                 return ""
...
#         # Require high preservation ratio
...
#         print(f"Processing PDF with CONTENT PRESERVATION MODE: {input_path}")
...
#                 output_path = f"{base_name}_gemini_content_preserved_extraction_failed.docx"
...
#             if not output_path:
#                 base_name = os.path.splitext(os.path.basename(input_path))[0]
#                 output_path = f"{base_name}_gemini_content_preserved_no_chunks.docx"
...
#             print(f"\nProcessing Chunk {i+1} of {len(text_chunks)} with STRICT CONTENT PRESERVATION")
#             chunk_outline = self.process_with_gemini(chunk_text, i + 1, len(text_chunks), pdf_full_text)
#             if chunk_outline:
#                 all_outlines.append(chunk_outline)
#             else:
#                 print(f"Chunk {i+1} was skipped due to content preservation concerns or processing failure.")
        
#         if not all_outlines:
#             print("No outlines were generated that passed content preservation checks.")
#             if not output_path:
#                 base_name = os.path.splitext(os.path.basename(input_path))[0]
#                 output_path = f"{base_name}_gemini_content_preserved_empty.docx"
#             self.create_docx_from_outline("No outlines could be generated that preserved original content integrity.", output_path)
#             return output_path

#         combined_outline_text = "\n".join(all_outlines)

#         print("\n--- Combined Content-Preserved Outline ---")
...
#         if not output_path:
#             base_name = os.path.splitext(os.path.basename(input_path))[0]
#             output_path = f"{base_name}_gemini_content_preserved_outline.docx"

#         if not parsed_structure and combined_outline_text:
#             print("Failed to parse the combined outline. DOCX will contain raw preserved content.")
...
#         else:
#             self.create_docx_from_outline("Content preservation checks prevented outline generation.", output_path)
        
#         return output_path

# def main():
#     print("Gemini CONTENT-PRESERVING Outline Converter")
#     print("=" * 60)
#     print("🔒 CONTENT PRESERVATION MODE ACTIVE")
#     print("✓ Original text will be preserved exactly")
#     print("✓ No paraphrasing, summarizing, or alterations")
#     print("✓ Citations and technical terms maintained")
#     print("✓ Only formatting and hierarchical organization")
#     print("-" * 60)
...
#         if result_path:
#             print(f"\n✅ SUCCESS: Content-preserved outline created at: {result_path}")
#             print("📋 Remember: This tool preserves original content exactly as written.")
#         else:
#             print("\n❌ FAILED: Could not process the file.")
#     except Exception as e:
#         print(f"\n❌ ERROR during processing: {e}")
    
#     print("\nThank you for using the Gemini Content-Preserving Converter!")
...
# # UPDATED CODE AS OF 12:17AM MAY 25 2025

# changelog: better preservation of content, less strict than previous code
...
#             else:
#                 print(f"WARNING: Chunk {chunk_num} outline FAILED content preservation check. Content may have been altered.")
#                 print("However, the output will still be included as per user request.")
...
#         # Require high preservation ratio - ADJUST THIS VALUE WITH CAUTION IF NEEDED
#         return preservation_ratio >= 0.8 # Or a lower value if you accept less strict preservation
...
#             else:
#                 paragraph.add_run("No structured content could be generated or parsed. The document might be image-based, encrypted, or content preservation checks prevented outline generation.").font.size = Pt(BODY_FONT_SIZE)
...
#             print(f"\nProcessing Chunk {i+1} of {len(text_chunks)} with STRICT CONTENT PRESERVATION")
#             chunk_outline = self.process_with_gemini(chunk_text, i + 1, len(text_chunks), pdf_full_text)
#             # The change is here: chunk_outline is always added if not empty, regardless of preservation check.
#             if chunk_outline:
#                 all_outlines.append(chunk_outline)
#             else:
...
#             self.create_docx_from_outline(parsed_structure, output_path)
#         else:
#             self.create_docx_from_outline("Content preservation checks prevented structured outline generation, or Gemini returned an unparseable response. The raw combined text might be limited.", output_path)
...
#             print(f"\n✅ SUCCESS: Content-preserved outline created at: {result_path}")
#             print("📋 Remember: This tool preserves original content exactly as written.")
#             print("⚠️ Note: If 'Content preservation check FAILED' warnings appeared, some output might be less faithful to original.")
#         else:
#             print("\n❌ FAILED: Could not process the file.")
#     except Exception as e:
#         print(f"\n❌ ERROR during processing: {e}")
...
# UPDATED CODE AS OF 12:45AM MAY 25 2025

# CHANGELOG: may change data if any errors found, lexical errors and semantics are taken care of.
import os
import re
import fitz  # PyMuPDF
import google.generativeai as genai
